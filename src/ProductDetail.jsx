...
function ProductDetail() {
  ...
  const initialProduct = products[id] || products['mix-berry-delight']
  
  // Always start with null - we'll fetch from API immediately
  // Only use hardcoded data as fallback if API fails
  ...
  useEffect(() => {
    ...
    async function fetchProductDetail() {
      ...
      setIsLoading(true)
      setProduct(null) // Clear any previous product data immediately
      ...
      try {
        ...
        if (error.name !== 'AbortError') {
          ...
          // Only fall back to hardcoded data if API fails (for legacy slug routes)
  ...
  // Don't render if loading and no product yet - show loading state
  ...
  // Don't render if no product at all (shouldn't happen, but safety check)
  if (!product) {
    return (
      <div className="product-detail-page">
